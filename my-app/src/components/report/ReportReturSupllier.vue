...
export default {
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'Supplier CD', value: 'kode_supplier' },
  ...
  methods: {
    async searchReport() {
      await this.get_report_returSupplier()
    },
    async get_report_returSupplier() {
      try {
        const response = await api.get('/t_retur_supplier/report');
    ...
    exportToExcel() {
      ...
      XLSX.writeFile(wb, "Rpt_retursupplier.xlsx");
    }
  }
}
</script>
